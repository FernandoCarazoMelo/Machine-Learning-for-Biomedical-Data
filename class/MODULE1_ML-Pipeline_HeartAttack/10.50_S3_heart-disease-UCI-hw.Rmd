...
p1 <- ggplot(data = data, aes(x = age, fill = target)) +
      geom_density(alpha = 0.5) +
      scale_fill_manual(values = c("gray50", "orangered2")) +
      geom_rug(aes(color = target), alpha = 0.5) +
      scale_color_manual(values = c("gray50", "orangered2")) +
      theme_bw()
p2 <- ggplot(data = data, aes(x = target, y = age, color = target)) +
      geom_boxplot() +
      geom_jitter(alpha = 0.3, width = 0.15) +
      scale_color_manual(values = c("gray50", "orangered2")) +
      theme_bw()
final_plot <- ggarrange(p1, p2, legend = "top")
final_plot <- annotate_figure(final_plot, top = "Heart Attack")
...
table(data$target) %>% prop.table()

# stratified sampling with the rsample package
set.seed(123)
split_strat  <- initial_split(data, prop = 0.8, strata = "target")
datos_train  <- training(split_strat)
datos_test   <- testing(split_strat)

# consistent response ratio between train & test
table(datos_train$target) %>% prop.table()

table(datos_test$target) %>% prop.table()
...
chol_log <- log(data$chol)

plot(data$chol, log(data$chol))

hist(data$chol, 100)


hist(log(data$chol), 100)

library(forecast)

hist(BoxCox(data$chol, -0.2), 100)
...
objeto_recipe <- recipe(target ~ ., data = datos_train)
objeto_recipe

# 2

objeto_recipe <- objeto_recipe %>% step_meanimpute(all_numeric())
...
vis_miss(datos_train_prep, cluster = T)
...
```{r}
library(caret)
data  %>% nearZeroVar(saveMetrics = TRUE)
...
# objeto_recipe <- objeto_recipe %>% step_dummy(all_nominal(), -all_outcomes(), one_hot = T)
objeto_recipe
...
1. **Adjustment / training**: consists of applying a machine learning algorithm to the training data so that the model learns.

2. **Evaluation / Validation**: The goal of a predictive model is not to be able to predict observations that are already known, but new observations that the model has not seen. In order to estimate the error that a model makes, it is necessary to resort to validation strategies, among which stand out test set, bootstrap and cross validation.

3. **Optimization of hyperparameters**: many machine learning algorithms contain in their equations one or more parameters that are not learned with the data, these are known as hyperparameters. For example, linear SVM has the cost hyperparameter C. There is no way of knowing in advance what the exact value of a hyperparameter is that gives rise to the best model, so validation strategies must be used to compare different values.

4. **Prediction**: Once the model is created, it is used to predict new observations.
...
First, we fit a model based on a linear support vector machine that predicts heart attack based on all available predictors after preprocessing. Except for the algorithm name, all other arguments to the `train` function and svmLinear's own are left by default.
...
# The resampling method: "boot", "boot632", "optimism_boot", "boot_all", "cv", "repeatedcv", "LOOCV", "LGOCV" (for repeated training/test splits), "none" (only fits one model to the entire training set), "oob" (only for random forest, bagged trees, bagged earth, bagged flexible discriminant analysis, or conditional tree forest models),
...
The object returned by train () contains multiple elements and a lot of information. At the moment, $ finalModel is analyzed, which is the final model obtained after the adjustment. 
...
[glmnet help](https://web.stanford.edu/~hastie/glmnet/glmnet_alpha.html)
...
> Your turn: find the best hyperparameters for a random forest, measuring Accuracy with a repeated CV (folds = 5, repeats = 5)


> find the best hyperparameters for a elasticnet logistic regression (glmnet), measuring Accuracy with a repeated CV (folds = 5, repeats = 10)
...
Adjustment, optimization and validation of the model

The ranger caret method uses the ranger () function of the ranger package. This algorithm has 3 hyperparameters:

- mtry: number of randomly selected predictors in each tree.

- min.node.size: minimum size that a node must have in order to be divided.

- splitrule: split criteria.

Although caret also includes the rf method with the rf () function of the randomForest package, the latter only allows optimizing the mtry hyperparameter.
...
> your turn: calculate median of accuracy
